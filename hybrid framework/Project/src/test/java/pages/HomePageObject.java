...
public class HomePageObject extends HomePageLocators{
    ...
    String offersButtonClick="clicked on Offers button";

    String yatraLogoClick="clicked on Yatra logo";

    String holidaysOptionClick="clicked on Holidays option";
    String hotelsOptionClick="clicked on Hotels option";

    String roundTripClick="clicked on Round Trip";
    ...
    public void clickOnOffersButton(WebDriver driver, ExtentTest test)
    {
        try {
            webDriverHelper = new WebDriverHelper(driver);

            webDriverHelper.waitForElementToBeVisible(getOfferButton(), 10);
            webDriverHelper.clickOnElement(getOfferButton());
            test.log(Status.PASS, offersButtonClick);
            LoggerHandler.logInfo(offersButtonClick);
        } catch (Exception e) {
            test.log(Status.FAIL, "not "+offersButtonClick);
            LoggerHandler.logError("not "+offersButtonClick);
        }
    }

    public void clickOnYatraLogo(WebDriver driver, ExtentTest test)
    {
        try {
            webDriverHelper = new WebDriverHelper(driver);

            webDriverHelper.clickOnElement(getYatraLogo());
            test.log(Status.PASS, yatraLogoClick);
            LoggerHandler.logInfo(yatraLogoClick);
        } catch (Exception e) {
            test.log(Status.FAIL, "not "+yatraLogoClick);
            LoggerHandler.logError("not "+yatraLogoClick);
        }
    }
    ...
    public void clickOnRoundTrip(WebDriver driver, ExtentTest test)
    {
        try {
            webDriverHelper = new WebDriverHelper(driver);

            webDriverHelper.waitForElementToBeVisible(getRoundTrip(), 10);
            webDriverHelper.clickOnElement(getRoundTrip());
            test.log(Status.PASS, roundTripClick);
            LoggerHandler.logInfo(roundTripClick);
        } catch (Exception e) {
            test.log(Status.FAIL, "not "+roundTripClick);
            LoggerHandler.logError("not "+roundTripClick);
        }
    }
